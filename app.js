...
function customSwaggerErrorHandler(err, req, res, next) {
  ...
  if (err.failedValidation) {
    console.error(`Error code 400 sent to user ${req.auth && req.auth.email}. Message is "${err.message}"`);
    res.status(400).json({ success: false, status: 400, message: err.message, validationErrors: err.results });
  } else {
    console.error(`Error code ${err.statusCode || 500} sent to user ${req.auth && req.auth.email}. Message is "${err.message}"`);
